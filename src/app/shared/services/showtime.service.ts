...
export class ShowtimeService {
  ...
  getShowtimes(params?: any): Observable<any> {
    ...
    return this.http.get<any>(`${this.baseUrl}/list/showtime`, { params: httpParams, withCredentials: true })
      .pipe(map(r => r.success ? r : (() => { throw new Error(r?.message || 'Failed'); })()),
        catchError(e => this.handleError(e)));
  }

  getShowtimeById(id: string): Observable<any> {
    return this.http.get<any>(`${this.baseUrl}/view/showtime/${id}`, { withCredentials: true })
      .pipe(map(r => r.success && r.data ? r.data : (() => { throw new Error(r?.message || 'Failed'); })()),
        catchError(e => this.handleError(e)));
  }

  save(itemData: any): Observable<any> {
    return this.http.post<any>(`${this.baseUrl}/submit/showtime`, itemData, { withCredentials: true })
      .pipe(map(r => r.success && r.data ? r.data : (() => { throw new Error(r?.message || 'Failed'); })()),
        catchError(e => this.handleError(e)));
  }

  delete(id: string): Observable<boolean> {
    return this.http.post<any>(`${this.baseUrl}/submit/showtime`, { stepSlug: 'v1', action: 'DELETE', id, formData: {} }, { withCredentials: true })
  ...
  bulkDelete(ids: string[]): Observable<boolean> {
    return this.http.post<any>(`${this.baseUrl}/delete`, { documentIds: ids, collectionName: 'showtimes' }, { withCredentials: true })
      .pipe(map(r => r.success ? true : (() => { throw new Error(r?.message || 'Failed'); })()),
        catchError(e => this.handleError(e)));
  }
