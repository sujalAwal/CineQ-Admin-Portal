...
export class ShowtimeStatusService {
  ...
  private readonly SLUG = 'showtime-statuses';
  ...
  bulkDelete(ids: string[]): Observable<boolean> {
    return this.http.delete<any>(`${this.baseUrl}/delete`, { body: { ids, formSlug: this.SLUG, collectionName: 'showtime_statuses' }, withCredentials: true })
      .pipe(map(r => r.success ? true : (() => { throw new Error(r?.message || 'Failed'); })()),
        catchError(e => this.handleError(e)));
  }
