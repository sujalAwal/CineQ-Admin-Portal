...
@Component({
  selector: 'app-showtime-status-modal',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, BaseModalComponent],
  templateUrl: './showtime-status-modal.component.html',
  styleUrls: ['./showtime-status-modal.component.scss']
})
export class ShowtimeStatusModalComponent implements OnInit, OnChanges {
  ...
  constructor(private fb: FormBuilder, private service: ShowtimeStatusService, private toastr: ToastrService, private cdr: ChangeDetectorRef) {}
  ...
  private initializeForm(): void {
    this.form = this.fb.group({
      code: ['', [Validators.required, Validators.minLength(1), Validators.maxLength(2), Validators.pattern(/^[A-Z0-9]+$/)]],
  ...
  private updateModalConfig(): void {
    if (this.item) {
      this.modalConfig.title = 'Edit Showtime Status';
      this.modalConfig.primaryButtonText = 'Update';
    } else {
      this.modalConfig.title = 'Add Showtime Status';
