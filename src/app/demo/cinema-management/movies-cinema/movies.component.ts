...
import { MoviesCinemaModalComponent } from '../../../shared/components/movies-cinema-modal/movies-cinema-modal.component';
...
import { MoviesCinemaService } from 'src/app/shared/services/movies-cinema.service';
import { PaginatedApiResponse } from '../../../shared/interfaces/genre.interface';

@Component({
  selector: 'app-movies-cinema',
  imports: [CommonModule, SharedModule, DataTableComponent, MoviesCinemaModalComponent, ConfirmationModalComponent],
  templateUrl: './movies.component.html',
  styleUrls: ['./movies.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class MoviesCinemaComponent implements OnInit, OnDestroy {

  tableConfig: TableConfig = {
    title: 'Movies Management',
    entityName: 'Movie',
    apiEndpoint: '/api/v1/list/movies',
    searchable: true,
    paginated: true,
    pageSize: 20,
    sortable: true,
    ...
    columns: [
      ...
      { header: 'Poster', field: 'poster', type: 'image', sortable: false, width: '80px', align: 'center' },
      { header: 'Duration', field: 'duration', type: 'text', sortable: false, width: '100px' },
  ...
  constructor(private toastService: ToastService, private service: MoviesCinemaService, private cdr: ChangeDetectorRef) {}
  ...
  loadData(additionalFilters?: Partial<any>) {
    ...
    this.service.getList(requestParams).pipe(takeUntil(this.destroy$)).subscribe({
      next: (response: PaginatedApiResponse<any>) => {
        // Extract and flatten from nested structure: { data: [{ "movies": [...] }] }
        this.moviesData = response.data.flatMap((item: any) => {
  ...
  onSort(sortInfo: { field: string, order: 'asc' | 'desc' }) { this.currentFilters = { ...this.currentFilters, sortBy: sortInfo.field, sortDirection: sortInfo.order, page: 1 }; this.loadData(); }
  ...
  onToggleChange(event: { item: any, field: string, value: boolean }) {
    ...
    operation([event.item.id]).pipe(takeUntil(this.destroy$)).subscribe({
      next: (success) => {
        if (success) {
          const message = event.value ? `"${event.item.title}" is now active.` : `"${event.item.title}" has been deactivated.`;
  ...
  private updateInList(id: string, updates: Partial<any>) {
    this.moviesData = this.moviesData.map(item => item.id === id ? { ...item, ...updates } : item);
  }
  ...
  private showBulkConfirmation(operation: 'enable' | 'disable' | 'delete', selectedIds: string[]) {
    ...
    if (operation === 'enable') {
      this.confirmationConfig = {
        title: 'Enable Movies',
        message: `Are you sure you want to <strong>enable</strong> ${count} movie(s)?`,
      ...
      this.confirmationConfig = {
        title: 'Disable Movies',
        message: `Are you sure you want to <strong>disable</strong> ${count} movie(s)?`,
      ...
      this.confirmationConfig = {
        title: 'Delete Movies',
        message: `Are you sure you want to <strong>delete</strong> ${count} movie(s)?<br><small class="text-danger">This action cannot be undone.</small>`,
        icon: 'ti ti-trash-x',
        iconColor: 'danger',
        confirmText: 'Delete',
        cancelText: 'Cancel',
  ...
  onItemSaved(itemData: any) {
    if (itemData.id) {
      this.toastService.success(`"${itemData.title}" has been updated successfully!`, 'Updated');
    } else {
      this.toastService.success(`"${itemData.title}" has been created successfully!`, 'Created');
    }
    this.onModalClosed();
    this.loadData();
  }
  ...
  onDeleteConfirmed() {
    if (this.itemToDelete) {
      ...
      this.service.delete(this.itemToDelete.id).subscribe({
        next: () => {
          this.toastService.success(`Movie "${this.itemToDelete!.title}" has been deleted successfully!`, 'Deleted');
          this.loadData();
    ...
    if (this.bulkOperation.type && this.bulkOperation.selectedIds.length > 0) {
      ...
      if (operation === 'enable') {
        this.service.enable(selectedIds).subscribe({
          next: () => { this.toastService.activated(`${selectedIds.length} movie(s) enabled!`, 'Enabled'); this.loadData(); this.resetBulkOperation(); },
          error: () => this.resetBulkOperation()
        });
      } else if (operation === 'disable') {
        this.service.disable(selectedIds).subscribe({
          next: () => { this.toastService.inactive(`${selectedIds.length} movie(s) disabled!`, 'Disabled'); this.loadData(); this.resetBulkOperation(); },
          error: () => this.resetBulkOperation()
        });
      } else if (operation === 'delete') {
        this.service.bulkDelete(selectedIds).subscribe({
          next: () => { this.toastService.success(`${selectedIds.length} movie(s) deleted!`, 'Deleted'); this.loadData(); this.resetBulkOperation(); },
          error: () => this.resetBulkOperation()
        });
      }
    }
