...
import { SeatStatusModalComponent } from '../../../shared/components/seat-status-modal/seat-status-modal.component';
...
@Component({
  ...
  imports: [CommonModule, SharedModule, DataTableComponent, SeatStatusModalComponent, ConfirmationModalComponent],
  templateUrl: './seat-status.component.html',
  styleUrls: ['./seat-status.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class SeatStatusComponent implements OnInit, OnDestroy {

  tableConfig: TableConfig = {
    title: 'Seat Status Management',
    entityName: 'Seat Status',
    apiEndpoint: '/api/v1/list/seat-status',
    searchable: true,
    paginated: true,
    pageSize: 20,
    sortable: true,
  ...
  constructor(
    private service: SeatStatusService,
  ...
  private updateInList(id: string, updates: Partial<any>): void {
    this.seatStatusData = this.seatStatusData.map(item => item.id === id ? { ...item, ...updates } : item);
  }
