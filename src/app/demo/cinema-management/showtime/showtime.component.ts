...
export class ShowtimeComponent implements OnInit, OnDestroy {
  ...
  tableConfig: TableConfig = {
    ...
    columns: [
      ...
      { header: 'Screen', field: 'screenId', type: 'text', sortable: true, width: '150px' },
  ...
  loadData(additionalFilters?: Partial<any>) {
    ...
    this.service.getShowtimes(requestParams).pipe(takeUntil(this.destroy$)).subscribe({
  ...
  private editItem(item: any) {
    ...
    this.service.getShowtimeById(item.id).subscribe({
      next: (fullItem) => {
