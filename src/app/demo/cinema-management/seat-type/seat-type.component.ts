...
import { SeatTypeModalComponent } from '../../../shared/components/seat-type-modal/seat-type-modal.component';
...
@Component({
  ...
  imports: [CommonModule, SharedModule, DataTableComponent, SeatTypeModalComponent, ConfirmationModalComponent],
...
export class SeatTypeComponent implements OnInit, OnDestroy {
  ...
  tableConfig: TableConfig = {
    title: 'Seat Types Management',
    entityName: 'Seat Type',
    apiEndpoint: '/api/v1/list/seat-type',
    searchable: true,
    paginated: true,
    pageSize: 20,
    sortable: true,
  ...
  constructor(
    private service: SeatTypeService,
  ...
  private updateInList(id: string, updates: Partial<any>): void {
    this.seatTypeData = this.seatTypeData.map(item => item.id === id ? { ...item, ...updates } : item);
  }
