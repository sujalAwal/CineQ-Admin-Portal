...
import { CrewRolesModalComponent } from '../../../shared/components/crew-roles-modal/crew-roles-modal.component';
...
import { CrewRolesService } from 'src/app/shared/services/crew-roles.service';
import { PaginatedApiResponse } from '../../../shared/interfaces/genre.interface';

@Component({
  selector: 'app-crew-roles',
  imports: [CommonModule, SharedModule, DataTableComponent, CrewRolesModalComponent, ConfirmationModalComponent],
  templateUrl: './crew-roles.component.html',
  styleUrls: ['./crew-roles.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class CrewRolesComponent implements OnInit, OnDestroy {

  tableConfig: TableConfig = {
    title: 'Crew Roles Management',
    entityName: 'Crew Role',
    apiEndpoint: '/api/v1/list/crew-roles',
    searchable: true,
    paginated: true,
    pageSize: 20,
    sortable: true,
    ...
    columns: [
      ...
      { header: 'Slug', field: 'slug', type: 'text', sortable: false, width: '180px' },
      { header: 'Description', field: 'description', type: 'text', sortable: false, width: '250px', maxLength: 50 },
  ...
  constructor(private toastService: ToastService, private service: CrewRolesService, private cdr: ChangeDetectorRef) {}
  ...
  loadData(additionalFilters?: Partial<any>) {
    ...
    this.service.getList(requestParams).pipe(takeUntil(this.destroy$)).subscribe({
      next: (response: PaginatedApiResponse<any>) => {
        // Extract and flatten from nested structure: { data: [{ "crew-roles": [...] }] }
        this.crewRolesData = response.data.flatMap((item: any) => {
          const records = item['crew-roles'] || item['crew_roles'] || item['crewRoles'] || item['crew-role'] || [];
  ...
  private updateInList(id: string, updates: Partial<any>) {
    this.crewRolesData = this.crewRolesData.map(item => item.id === id ? { ...item, ...updates } : item);
  }
