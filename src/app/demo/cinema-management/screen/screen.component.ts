...
import { ScreenModalComponent } from '../../../shared/components/screen-modal/screen-modal.component';
...
@Component({
  selector: 'app-screen',
  imports: [CommonModule, SharedModule, DataTableComponent, ScreenModalComponent, ConfirmationModalComponent],
...
export class ScreenComponent implements OnInit, OnDestroy {
  ...
  tableConfig: TableConfig = {
    ...
    entityName: 'Screen',
    apiEndpoint: '/api/v1/list/screen',
    searchable: true,
    paginated: true,
    pageSize: 20,
    sortable: true,
    ...
    columns: [
      ...
      { header: 'Screen Name', field: 'screenName', type: 'text', sortable: true, width: '200px' },
      { header: 'Type', field: 'screenType', type: 'text', sortable: true, width: '150px' },
      { header: 'Rows', field: 'rows', type: 'number', sortable: false, width: '80px', align: 'center' },
      { header: 'Columns', field: 'columns', type: 'number', sortable: false, width: '80px', align: 'center' },
  ...
  constructor(private toastService: ToastService, private service: ScreenService, private cdr: ChangeDetectorRef) {}
  ...
  onToggleChange(event: { item: any, field: string, value: boolean }) {
    ...
    operation([event.item.id]).pipe(takeUntil(this.destroy$)).subscribe({
      next: (success) => {
        if (success) {
          const message = event.value ? `"${event.item.screenName}" is now active.` : `"${event.item.screenName}" has been deactivated.`;
  ...
  private updateInList(id: string, updates: Partial<any>) {
    this.screenData = this.screenData.map(item => item.id === id ? { ...item, ...updates } : item);
  }
  ...
  onItemSaved(itemData: any) {
    if (itemData.id) {
      this.toastService.success(`"${itemData.screenName}" has been updated successfully!`, 'Updated');
    } else {
      this.toastService.success(`"${itemData.screenName}" has been created successfully!`, 'Created');
    }
  ...
  private deleteItem(item: any) {
    ...
    this.confirmationConfig = {
      title: 'Delete Item',
      message: `Are you sure you want to delete <strong>"${item.screenName}"</strong>?<br><small class="text-muted">This action cannot be undone.</small>`,
      icon: 'ti ti-trash-x',
      iconColor: 'danger',
      confirmText: 'Delete',
      cancelText: 'Cancel',
  ...
  onDeleteConfirmed() {
    if (this.itemToDelete) {
      ...
      this.service.delete(this.itemToDelete.id).subscribe({
        next: () => {
          this.toastService.success(`Item "${this.itemToDelete!.screenName}" has been deleted successfully!`, 'Deleted');
          this.loadData();
